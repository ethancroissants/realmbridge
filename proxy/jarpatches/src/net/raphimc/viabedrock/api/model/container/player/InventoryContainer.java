...
public class InventoryContainer extends Container {
    ...
    protected void onSlotChanged(final int slot, final BedrockItem oldItem, final BedrockItem newItem) {
        ...
        // VP+: only notify the server when the held item actually changed. Every full
        // INVENTORY_CONTENT push lands here for the held slot; echoing MOB_EQUIPMENT
        // back for identical items makes the server re-broadcast the inventory,
        // creating a content-flood feedback loop.
        if (slot == this.selectedHotbarSlot && oldItem.isDifferent(newItem)) {
