...
public class WorldPackets {
    ...
    private static final PacketHandler UPDATE_BLOCK_HANDLER = wrapper -> {
        ...
        final long vppBrokenAgo = net.raphimc.viabedrock.experimental.VppResync.brokenAgoMillis(wrapper.user(), position);
        if (vppBrokenAgo >= 0 && remappedBlock.keyInt() != net.raphimc.viabedrock.protocol.data.ProtocolConstants.JAVA_AIR_ID) {
            net.raphimc.viabedrock.ViaBedrock.getPlatform().getLogger().log(java.util.logging.Level.INFO,
                    "[VP+ diag] break REVERTED at " + position + " after " + vppBrokenAgo + "ms (javaState=" + remappedBlock.keyInt() + ")");
        }
