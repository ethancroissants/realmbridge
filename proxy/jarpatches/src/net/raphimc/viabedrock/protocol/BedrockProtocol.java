        ...
        if (direction == Direction.CLIENTBOUND) {
            ...
            switch (packet) {
                case ADD_ITEM_ENTITY, TAKE_ITEM_ENTITY, CONTAINER_OPEN, ITEM_STACK_RESPONSE ->
                        ViaBedrock.getPlatform().getLogger().log(Level.FINE, "[VP+ diag] recv " + packet + " (state=" + serverState + ")");
                case INVENTORY_TRANSACTION -> {
                    final ByteBuf vppBuf = ((PacketWrapperImpl) wrapper).getInputBuffer();
                    final int vppLen = Math.min(256, vppBuf.readableBytes());
                    ViaBedrock.getPlatform().getLogger().log(Level.FINE, "[VP+ diag] recv INVENTORY_TRANSACTION (state=" + serverState
            ...
            if (serverState != State.PLAY && !BEFORE_PLAY_STATE_WHITELIST.contains(packet)) { // Bedrock client ignores most packets before receiving the START_GAME packet
                // VP+ patch: pass inventory state packets to the handlers during CONFIGURATION.
                // Handlers drop them gracefully pre-item-palette and apply them post-palette.
                final boolean vppInventoryBaseline = serverState == State.CONFIGURATION
                        && (packet == ClientboundBedrockPackets.INVENTORY_SLOT || packet == ClientboundBedrockPackets.INVENTORY_CONTENT);
                if (!vppInventoryBaseline) {
